...
func (t *MysqlTest) RunOnce() {
	...
	counts_to_check := []int64{0, 10, 50, 100, 150, 200, 250, 300, 600, 1200, 2400}
	if err != nil {
...
func (t *MysqlTest) WriteResult(testname string, passed bool, description string) {
	...
	path := fmt.Sprintf("%s.txt", filepath.Join(t.filedirectory, "/", testname))
...
func (t *MysqlTest) CheckReplication() (int64, string, error) {
	...
	if slave_io_running == "" || seconds_behind_master == -1 {
		description = fmt.Sprintf("Slave status and/or seconds_behind_master could not be determined.", slave_io_running, seconds_behind_master)
